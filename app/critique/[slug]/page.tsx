...
export default async function ReviewPage({
  ...
  const image = r.mainImage
    ? urlFor(r.mainImage).width(1600).quality(90).url()
    : '/new-religion.jpg'

  const readingTime = r.body
    ? getReadingTime(r.body)
    : Math.max(
        1,
        Math.ceil(
          fallbackParagraphs
  ...
  return (
    <article className="review-page">
      <header>
        <p className="kicker">
          CRITIQUE / {r.director.toUpperCase()} / {r.year}
        </p>

        <h1>{r.articleTitle}</h1>
      ...
      <div className="article-body">
        {r.body ? (
          <PortableText value={r.body as never} />
        ) : (
